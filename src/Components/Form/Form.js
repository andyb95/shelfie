...
export default class Form extends Component{
  ...
  componentDidUpdate(oldProps){
    let {product_id, name, price, imgurl} = this.props.product
    if (oldProps.product.product_id !== this.props.product.product_id) {
      this.setState({
        product_id, name, price, imgurl, isEditing: true
      })
    }
  }
  ...
  addToInventory= async ( res) => {
    const body = {...this.state}
    const newItem = await axios.post('/api/product', body)
    if (newItem[0]){
  ...
  editProduct = () => {
    const {product_id, name, price, imgurl} = this.state
    console.log(product_id, name, price, imgurl)
    if (name) {
      const product = {
        name,
        price,
      ...
      <div className = 'form'>
        ...
        {!this.props.isEditing ? (
          ...
          <div className = 'form-buttons'>
            <button className = 'form-button' onClick={this.cancel}>CLEAR</button>
              
            <button className = 'form-button' onClick={this.addToInventory}>ADD TO INVENTORY</button>
          </div>  
